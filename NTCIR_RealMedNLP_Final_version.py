...
results = []
for i in range(5):
    results.append(evaluation_testset(str(i)))

# %%
# xarrayのDatasetを使い，3次元データにする
import xarray as xr
Ds_results = xr.concat([df.to_xarray() for df in results], dim="dataset")

# %%
#平均を計算する
cols = ['type', 'num_entities','num_predictions', 'num_correct', 'precision', 'recall', 'f_value']
frame = pd.DataFrame(index=cols).T#転置するのでカラムになる

for i in range(len(results[0])):
    Ds_mean = Ds_results.isel(index=i).mean()#インデックスの上から順に平均を取る
    result = Ds_mean.to_array().to_dataframe('variable').T.iloc[1:2,:]#データフレームに直す
    tag_type = Ds_results.isel(index=i)['type'].to_dataframe().iloc[0,1]#タイプを取得する
    result.insert(0, 'type', tag_type)
    list_values = result.values.tolist()[0]
    df = pd.DataFrame(list_values, index= cols).T
...
frame['num_entities'] = frame['num_entities'].map(lambda x: '{0:.0f}'.format(x))
frame['num_predictions'] = frame['num_predictions'].map(lambda x: '{0:.1f}'.format(x))
frame['num_correct'] = frame['num_correct'].map(lambda x: '{0:.1f}'.format(x))
frame
